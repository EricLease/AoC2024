...
static IEnumerable<PageOrderingRule> GetViolations(List<PageOrderingRule> rules, List<int> updatePages)
    => rules.Where(r =>
    updatePages.IndexOf(r.Before) > -1 &&
    updatePages.IndexOf(r.After) > -1 &&
    updatePages.LastIndexOf(r.Before) > updatePages.IndexOf(r.After));

var lines = GetLines(_puzzleInput);
var lineIdx = 0;
var rules = new List<PageOrderingRule>();

for (; lineIdx < lines.Count; lineIdx++)
{
    var rule = PageOrderingRule.GenerateFromInput(lines[lineIdx]);
...
for (; lineIdx < lines.Count; lineIdx++)
{
    updates.Add(PageUpdate.GenerateFromInput(lines[lineIdx]));
}

var total = 0;

foreach (var updatePages in updates.Select(u => u.Pages))
{
    var violations = GetViolations(rules, updatePages);
    var violated = violations.Any();

    while (violations.Any())
    {
        foreach (var violation in violations)
        {
            var lastBeforeIdx = updatePages.LastIndexOf(violation.Before);
            var firstAfterIdx = updatePages.IndexOf(violation.After);

            while (lastBeforeIdx > firstAfterIdx)
            {
                updatePages.RemoveAt(lastBeforeIdx);
                updatePages.Insert(firstAfterIdx, violation.Before);

                lastBeforeIdx = updatePages.LastIndexOf(violation.Before);
                firstAfterIdx = updatePages.IndexOf(violation.After);
            }
        }

        violations = GetViolations(rules, updatePages);
    }

    if (violated) total += updatePages[updatePages.Count / 2];
}

Console.WriteLine(total);

internal record struct PageOrderingRule(int Before, int After)
{
    internal const char Delimeter = '|';

    internal static PageOrderingRule? GenerateFromInput(string? ruleInput)
    {
        var parts = ruleInput?.Split(Delimeter, StringSplitOptions.RemoveEmptyEntries);
...
internal record struct PageUpdate(List<int> Pages)
{
    internal const char Delimeter = ',';

    internal static PageUpdate GenerateFromInput(string pageOrderInput)
        => new(
            pageOrderInput
