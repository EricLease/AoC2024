...
const string _puzzleInput =
    ...
    """;
const long _prizeOffset = 10000000000000;
...
forward (along the Y axis)

Each machine contains one prize; the claw must be positioned exactly above the prize on both the X and Y axes.
...
This list describes the button configuration and prize location of four different machines.
...
For the second and fourth machines, there is no combination of A and B presses that will ever win a prize
For the third machine, pushing the A button 38 times and the B button 86 times is the cheapest and would cost 200 tokens
...
rearrange 1st eqn to solve for n:
n = (px - m*bx) / ax

substitute for n in 2nd eqn, and rearrange to solve for m:
m = (py*ax - px*ay) / (ax*by - ay*bx)

Plug in actual values for the machine to find n and m.
If they are whole numbers (floor(n) == n && floor(m) == m) then it's a valid solution,
so just multiply n * 3, add m (*1) to get cost for machine.
Sum all machine costs that are valid.

---- Part two, add (10000000000000, 10000000000000) to every prize position, the raw input doesn't change
- same as part1, just need to modify prize input after parse, create custom point struct to hold long X/Y
and use doubles instead of floats in calculations to prevent rounding errors and whatnot.
 */

Console.WriteLine($"Total: {Solve(Parse(_input.GetLines()))}");

static long Solve(IEnumerable<Set> sets)
{
    var total = 0L;

    foreach (var set in sets)
    {
        var px = (double)set.Prize.X;
        var py = (double)set.Prize.Y;
        var ax = (double)set.A.X;
        var ay = (double)set.A.Y;
        var bx = (double)set.B.X;
        var by = (double)set.B.Y;
        var m = (py * ax - px * ay) / (ax * by - ay * bx);
        var n = (px - m * bx) / ax;

        if (_debug) Console.Write($"n = {n}, m = {m}");
        ...
        if (n == Math.Floor(n) && m == Math.Floor(m)) 
        {
            ...
            if (_debug) 
            {
                Console.WriteLine($"Solution found: [{(long)(n * 3 + m)}]");
                Console.WriteLine($"\tn = {(long)n}, m = {(long)m}");
            }
        } else if(_debug) Console.WriteLine("No solution");
    }

    return total;
}

static List<Set> Parse(IEnumerable<string> lines)
{
    var i = 0;
    ...
    while (i < lines.Count())
    {
        var a = ExtractPoint(lines.ElementAt(i++));
        var b = ExtractPoint(lines.ElementAt(i++));
        var prize = ExtractPoint(lines.ElementAt(i++));

        sets.Add(new(a, b, new(prize.X + _prizeOffset, prize.Y + _prizeOffset)));
    }

    return sets;
}

static Point ExtractPoint(string rawLine)
{
    var parts = rawLine
    ...
    return new Point(
        ExtractPointPart(parts[0]),
        ExtractPointPart(parts[1]));
}

static int ExtractPointPart(string raw) => int.Parse(raw.Split(['+', '='])[1]);

internal readonly record struct Set(Point A, Point B, Point Prize);
