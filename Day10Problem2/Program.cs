...
--- Part Two ---
ROOT RATING: the number of distinct PATHs which begin at that ROOT. For example:

Single root; its root rating is 3 because there are exactly three distinct paths which begin at that position:
.....0.
..4321.
..5..2.
..6543.
...
Here is a map containing a single root with root rating 13:
..90..9
...1.98
...2..7
6543456
...
This map contains a single root with root rating 227
(121 distinct paths that lead to the 9 on the right edge and 106 that lead to the 9 on the bottom edge):
012345
123456
234567
345678
...
01329801
10456732
In their reading order, their root ratings are: 20, 24, 10, 4, 1, 4, 5, 8, 5
The sum of all root ratings in this larger example map is 81

Change from problem 1 -> 2 is just returning all "child paths" (List) from WalkPath instead of distinct (HashSet).
 */
...
#pragma warning disable CS0162 // Unreachable code detected
    _config[Map.MasterDebugKey] = false;
    _config[Map.DataDebugKey] = true;
    _config[Map.MasterPauseDebugKey] = false;
    _config[Map.RootPauseDebugKey] = false;
    _config[Map.PathPauseDebugKey] = false;
...
internal record struct Map
{
    ...
    // Expect 81: (20 + 24 + 10 + 4 + 1 + 4 + 5 + 8 + 5)
    ...
    internal void Evaluate()
    {
        ...
        for (var i = 0; i < MaxOffset; i++)
        {
            ...
            if (Data[i] == MinValue)
            {
                if (MasterDebug)
                {
                    ...
                    var paths = WalkPath(i, 0, [i]);

                    if (paths.Count > 0)
                    {

                        totalRootRating += paths.Count;
                        Console.ForegroundColor = ConsoleColor.DarkGreen;
                        Console.WriteLine($"Unique Root Path(s) Found ({paths.Count}): [{string.Join(", ", paths)}]");
                        Console.WriteLine($"Root Count: {++RootCount}");
                        Console.WriteLine($"Root Rating Subtotal: {totalRootRating}");
                    }
                    else
                    {
                ...
                totalRootRating += WalkPath(i, 0, []).Count;
            }
            else if (MasterDebug)
            {
                var oldColor = Console.ForegroundColor;
        ...
        Console.WriteLine($"\nTotal Root Rating: {totalRootRating}");
    }

    private List<int> WalkPath(int offset, int currValue, List<int> currPath)
    {
        if (MasterDebug && MasterPause)
        {
            Console.Clear();
        ...
        var pathEnds = new List<int>();
        var nextNeighbors = FindNextNeighbors(offset, ++currValue, currPath);

        foreach (var neighbor in nextNeighbors)
        {
            pathEnds.AddRange(WalkPath(neighbor, currValue, MasterDebug ? [.. currPath, neighbor] : []));
