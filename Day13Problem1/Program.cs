...
static int Solve(IEnumerable<Set> sets)
{
    var total = 0;

    foreach (var set in sets)
    {
        var px = (float)set.Prize.X;
        var py = (float)set.Prize.Y;
        var ax = (float)set.A.X;
        var ay = (float)set.A.Y;
        var bx = (float)set.B.X;
        var by = (float)set.B.Y;
        var m = (py * ax - px * ay) / (ax * by - ay * bx);
        var n = (px - m * bx) / ax;
        if (n == Math.Floor(n) && m == Math.Floor(m)) total += (int)(n * 3 + m);
    }
