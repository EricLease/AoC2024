...
Antinodes now occur at all locations in the bounds along the ab axis at intervals of d, starting
at and including a and b respectively.  This creates many more antinodes, 34 expected in total on the original map:
...
//    KEEP adding to antinode set coordinates:
//      (p1 - d*n), while in bounds, increment n, repeat
//      (p2 + d*n), while in bounds, increment n, repeat
//      [if >= (0, 0) && < (width, height)]
...
foreach (var (_, points) in pointLetters.Where(kvp => kvp.Value.Count > 1))
{
    for (var i = 0; i < points.Count - 1; i++)
    {
        ...
        for (var j = i + 1; j < points.Count; j++)
        {
            ...
            var anti = new Point(a.X, a.Y);
            var n = 0;
            
            do
            {
                n++;
                antinodeSet.Add(anti);
                anti = new Point(a.X - d.X * n, a.Y - d.Y * n);
            } while(anti.InBounds(width, height));

            anti = new Point(b.X, b.Y);
            n = 0;
            do
            {
                n++;
                antinodeSet.Add(anti);
                anti = new Point(b.X + d.X * n, b.Y + d.Y * n);
            } while (anti.InBounds(width, height));
        }
    }
}
