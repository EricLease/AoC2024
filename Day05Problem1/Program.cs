...
{
    Console.WriteLine($"Processing update: [{string.Join(PageUpdate.Delimeter, update.Pages)}]");

    var valid = true;

    foreach (var rule in rules)
    {
        if (update.Pages.IndexOf(rule.Before) < 0 ||
            update.Pages.IndexOf(rule.After) < 0 ||
            update.Pages.LastIndexOf(rule.Before) < update.Pages.IndexOf(rule.After))
        { 
            Console.Write('.');
            continue; 
        }

        Console.WriteLine($"\nRule violation: [{rule.Before}, {rule.After}]");
        valid = false;
        break;
    }
    ...
    var midValue = update.Pages[update.Pages.Count / 2];
    
    Console.WriteLine($"Update mid value: {midValue}");
    total += midValue;
}
